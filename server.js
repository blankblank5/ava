...
// ─── UPGRADED MOVES (replacing move index 0 when shrine collected) ───────────
...
// ─── BOSS REWARD (unlocked move index 3 upgrade) ─────────────────────────────
...
// ─── BOSS ─────────────────────────────────────────────────────────────────────
// Arena center at (120, 0, 0). Boss confined to arena radius 28.
...
// ─── SKY SHRINES ─────────────────────────────────────────────────────────────
// Each shrine is a floating island at high altitude. Only reachable by its element.
// Air needs 3x dash charges to get there. Others need specific mechanics.
const SHRINES = {
    AIR:   { x:   0, y: 80, z: -140, collected: new Set() },
    EARTH: { x: 140, y: 55, z:    0, collected: new Set() },   // lower — reachable by meteor launching yourself up
    FIRE:  { x: -140, y: 65, z:   0, collected: new Set() },   // mid — reachable by fire blast knockback + jump
    WATER: { x:   0, y: 60, z:  140, collected: new Set() }    // mid — reachable by water whip / tsunami momentum
};
...
// Boss fires a rock projectile at a target player
...
// Boss ground stomp — spikes in ring around boss
function bossStomp() {
    ...
    for (let i = 0; i < count; i++) {
        ...
        setTimeout(() => {
            projectiles.push({
                id: 'bproj_' + (projCounter++), ownerId: 'BOSS',
                x: boss.x + Math.cos(angle) * r, y: 0,
                z: boss.z + Math.sin(angle) * r,
...
io.on('connection', (socket) => {
    ...
    socket.on('switchElement', (elName) => {
        ...
        // Re-apply shrine upgrade if this element has one
    ...
    socket.on('useMove', (index) => {
        ...
        // Determine the actual move to use (upgraded or boss reward override)
        let move;
        ...
        if (move.type === 'meteor') {
            ...
            projectiles.push({
                ...
                x: p.x + dir.x * aimDist + (Math.random()-0.5)*1.5,
                y: p.y + 30,
        ...
        if (move.type === 'groundSpikes') {
            ...
            for (let i = 0; i < spikeCount; i++) {
                ...
                setTimeout(() => {
                    projectiles.push({
                        id: 'proj_' + (projCounter++), ownerId: socket.id,
                        x: ax + (Math.random()-0.5)*0.4, y: 0,
                        z: az + (Math.random()-0.5)*0.4,
                        vx: 0, vy: 0, vz: 0, gravity: 0,
...
setInterval(() => {
    ...
    if (boss.alive) {
        ...
        // Find closest player IN or NEAR arena
        ...
        if (closest) {
            ...
            boss.vx += (dx/len) * spd * DELTA;
            boss.vz += (dz/len) * spd * DELTA;
            boss.yaw = Math.atan2(-dx, -dz);
        }
        // Friction
        boss.vx *= 0.85; boss.vz *= 0.85;
        boss.x += boss.vx * DELTA * 3;
        boss.z += boss.vz * DELTA * 3;
        ...
        if (now > boss.attackTimer) {
            ...
            if (closest) {
                ...
                if (r < 0.6) {
                    ...
                    for (let b = 0; b < 3; b++) {
                        setTimeout(() => { if (closest && players[closest.id]) bossFireAt(players[closest.id]); }, b * 250);
                    }
                } else {
                    bossStomp();
                }
        ...
        // Boss dead — respawn after 90 seconds
        ...
        if (now > boss.respawnTimer) {
            ...
            // Reset shrine collections for a new round
    ...
    for (const id in players) {
        ...
        if (now > p.rootUntil) {
            ...
            if (p.inputs.forward)  { moveX+=fx; moveZ+=fz; }
            if (p.inputs.backward) { moveX-=fx; moveZ-=fz; }
            if (p.inputs.right)    { moveX+=rx; moveZ+=rz; }
            if (p.inputs.left)     { moveX-=rx; moveZ-=rz; }
        ...
        if (!p.shrineCollected) {
            ...
            if (shrine && !shrine.collected.has(id)) {
                ...
                if (Math.sqrt(sdx*sdx+sdy*sdy+sdz*sdz) < 4) {
                    shrine.collected.add(id);
                    p.shrineCollected = true;
    ...
    for (let i = projectiles.length-1; i >= 0; i--) {
        ...
        // Boss projectiles hitting players
        if (!destroyed && pr.ownerId === 'BOSS') {
            for (const pid in players) {
                const t=players[pid];
                const dx=pr.x-t.x,dy=pr.y-(t.y+0.9),dz=pr.z-t.z;
        ...
        if (!destroyed && !pr.isSolid && !pr.isGroundSpike && pr.ownerId !== 'BOSS' && boss.alive) {
            ...
            if (Math.sqrt(dx*dx+dy*dy+dz*dz)<(pr.size*2+2.5)) {
                ...
                if (boss.hp <= 0) {
                    ...
                    // Give reward to all players who were in/near arena
                    for (const pid in players) {
                        const t = players[pid];
    ...
    // ── BUILD STATE ──────────────────────────────────────────────────────────
    // Send boss state and shrine states to clients
    const shrineState = {};
    for (const el in SHRINES) {
        const s = SHRINES[el];
        shrineState[el] = { x: s.x, y: s.y, z: s.z };
    }

    io.emit('gameState', {
        players, projectiles, slowMo,
        boss: {
            alive: boss.alive, hp: boss.hp, maxHp: boss.maxHp,
            x: boss.x, y: boss.y, z: boss.z, yaw: boss.yaw,
            phase: boss.phase
        },
        shrines: shrineState
    });
