...
// WALL geometry: BoxGeometry(6, 4, 1)
// Stored at CENTER. Bottom of wall = wallY - 2, top = wallY + 2.
// We fix wallY = max(p.y, 0) + 2 so it always sits on the ground.
const WALL_HALF_W = 3.0;  // half of width 6
const WALL_HALF_H = 2.0;  // half of height 4
const WALL_HALF_D = 0.5;  // half of depth 1
...
// Rotate world-space (dx,dz) into wall local space — matches Three.js mesh.rotation.y = yaw
...
function fromWallLocal(localX, localZ, yaw) {
    const cos = Math.cos(yaw), sin = Math.sin(yaw);
    return { wx: localX * cos - localZ * sin, wz: localX * sin + localZ * cos };
}
...
io.on('connection', (socket) => {
    ...
    socket.on('useMove', (index) => {
        ...
        if (move.type === 'wall') {
            const dist = 6;
            // Wall center Y = 2 units above the ground (wall is 4 tall, so bottom at y=0, top at y=4)
            // Clamp to ground level in case player is falling
            const wallGroundY = Math.max(p.y, 0);
            projectiles.push({
                id: "proj_" + (projCounter++), ownerId: socket.id,
                x: p.x + flat.x * dist,
                y: wallGroundY + WALL_HALF_H,   // center = ground + half-height
                z: p.z + flat.z * dist,
            ...
            for (let i = 0; i < spikeCount; i++) {
                ...
                setTimeout(() => {
                    projectiles.push({
                        ...
                        y: 0, z: aimZ + (Math.random() - 0.5) * 0.4,
                        vx: 0, vy: 0, vz: 0, gravity: 0,
                        life: 30, dmg: move.dmg, shape: 'spike',
                        color: 0x8b4513, size: 0.5, knockback: 0, rootTime: 2000, isGroundSpike: true
                    });
                }, delay);
            }
            return;
        ...
        if (move.type === 'whirlpool') {
            ...
            for (let i = 0; i < ringCount; i++) {
                ...
                const spawnDist = 3 + Math.random() * 1.5;
                setTimeout(() => {
                    projectiles.push({
                        id: "proj_" + (projCounter++), ownerId: socket.id,
                        x: p.x + Math.cos(angle) * spawnDist, y: 0.3,
                        z: p.z + Math.sin(angle) * spawnDist,
                        vx: Math.cos(angle) * 3, vy: 0, vz: Math.sin(angle) * 3,
                        gravity: 0, life: 120, dmg: move.dmg,
                        shape: 'whirlpool', color: 0x0088ff, size: 0.6,
                        knockback: 2.0, rootTime: 0
                    });
                }, i * 40);
            }
            return;
...
setInterval(() => {
    ...
    for (const id in players) {
        ...
        // Wall center is at (pr.x, pr.y, pr.z). pr.y = groundY + WALL_HALF_H
        // Player feet at p.y, eyes at p.y + 1.7. Player center for overlap = p.y + 0.9
        for (const pr of projectiles) {
            ...
            if (Math.abs(dy) > WALL_HALF_H) continue;   // above or below wall

            const { localX, localZ } = toWallLocal(dx, dz, pr.yaw);
            if (Math.abs(localX) < WALL_HALF_W && Math.abs(localZ) < WALL_HALF_D) {
                const penX = WALL_HALF_W - Math.abs(localX);
                const penZ = WALL_HALF_D - Math.abs(localZ);
        ...
        if (p.isJailed) {
            ...
            if (p.x > 13.5 && p.x < 26.5 && p.z > 13.5 && p.z < 26.5 && p.y < 10) {
                ...
                if (min === dx1) p.x = 13.5; else if (min === dx2) p.x = 26.5;
                else if (min === dz1) p.z = 13.5; else p.z = 26.5;
            }
        }

        if (p.y <= 0) {
            p.y = 0; p.vy = 0;
            if (p.inputs.jump && now > p.rootUntil) { p.vy = 6.0; p.inputs.jump = false; }
        }
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (slowMo.active) {
            ...
            if (pr.ownerId !== slowMo.owner && (!prOwner || prOwner.role !== 'owner')) pTimeScale = 0.05;
        }
        ...
        if (!pr.isSolid && pr.dmg > 0) {
            for (let j = projectiles.length - 1; j >= 0; j--) {
                ...
                if (Math.abs(localX) <= WALL_HALF_W && Math.abs(localZ) <= WALL_HALF_D) {
                    wall.hp--; if (wall.hp <= 0) wall.life = 0;
        ...
        if (!destroyed && pr.isGroundSpike) {
            for (const pid in players) {
                ...
                if (Math.sqrt(dx*dx + dz*dz) < 1.5 && t.y < 2.0) {
                    if (!t.godMode && pr.dmg > 0) {
                        t.hp -= pr.dmg;
                        t.rootUntil = Date.now() + pr.rootTime;
