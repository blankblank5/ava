...
const ELEMENTS = {
    AIR: { name: 'AIR', color: 0xffffff, moves: [
        ...
        { dmg: 20, cd: 3000, speed: 30, shape: 'sphere', size: 0.8, knockback: 2.0 },
        { dmg: 0, cd: 4000, type: 'dash', maxCharges: 3 },
        { dmg: 25, cd: 8000, speed: 20, shape: 'cone', size: 0.4, knockback: 3.0, count: 5, spread: 0.5 }
    ]},
    ...
    WATER: { name: 'WATER', color: 0x0088ff, moves: [
        ...
        { dmg: 25, cd: 3000, speed: 80, shape: 'sphere', size: 0.2 },
        { dmg: 5, cd: 5000, speed: 15, grav: -0.05, shape: 'sphere', count: 12, spread: 1.0, knockback: 0.5, size: 0.3 }
    ]},
...
io.on('connection', (socket) => {
    ...
    socket.on('useMove', (index) => {
        ...
        if (move.type === 'wall') {
            ...
            for(let i = -1; i <= 1; i++) {
                projectiles.push({
                    id: "proj_" + (projCounter++), ownerId: socket.id,
                    x: p.x + (dir.x * dist) + (Math.cos(p.yaw) * i * 2.5), 
                    y: p.y + 3, // Tall wall
                    z: p.z + (dir.z * dist) + (-Math.sin(p.yaw) * i * 2.5),
                    vx: 0, vy: 0, vz: 0, gravity: 0,
                    life: 300, // 10 Seconds
                    dmg: 0, shape: 'wall', color: elemStats.color, size: 1.0, isSolid: true, hp: 5 // 5 hits to break
                });
            }
            return;
        }
    ...
    socket.on('adminCommand', (data) => {
        ...
        // Owner only commands
        if (p.role === "owner" && data.action === 'giveAdmin') target.role = "admin";
        
        // General Admin commands
        if (data.action === 'nuke') { for(let id in players) { if(!players[id].godMode) { players[id].hp = 0; io.to(id).emit('death'); } } }
        else if (data.action === 'jail') { target.isJailed = !target.isJailed; if (target.isJailed) { target.x = 20; target.y = 1; target.z = 20; } }
...
setInterval(() => {
    ...
    for (const id in players) {
        ...
        // SOLID WALL COLLISION FOR PLAYERS
        for (const pr of projectiles) {
            if (pr.isSolid) {
                const dx = p.x - pr.x; const dz = p.z - pr.z;
                const dist = Math.sqrt(dx*dx + dz*dz);
                if (dist < 2.0 && p.y < pr.y + 3) { // Wall hit radius
                    const push = 2.0 - dist;
        ...
        if (!pr.isSolid && pr.dmg > 0) {
            for (let j = projectiles.length - 1; j >= 0; j--) {
                ...
                if (wall.isSolid && pr.ownerId !== wall.ownerId) {
                    ...
                    if (Math.sqrt(dx*dx + dy*dy + dz*dz) < 2.5) { // Hit the wall
                        wall.hp -= 1;
                        if (wall.hp <= 0) wall.life = 0; // Destroy wall
        ...
        if (!destroyed && !pr.isSolid) {
            for (const pid in players) {
                ...
                if (Math.sqrt(dx*dx + dy*dy + dz*dz) < (pr.size * 2 + 1.5)) {
                    ...
                    if (pr.shape !== 'flat') destroyed = true; 
                }
            }
        }
